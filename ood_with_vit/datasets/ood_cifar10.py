...
class OOD_CIFAR10(torchvision.datasets.CIFAR10):
    ...
    def __init__(
        ...
        id_data, id_targets = [], []
        self.id_class_to_idx = {in_dist: i for i, in_dist in enumerate(in_distribution_class_indices)}
        self.idx_to_id_class = {i: in_dist for i, in_dist in enumerate(in_distribution_class_indices)}
        for class_idx in in_distribution_class_indices:
            id_class_imgs = self.img_dict[class_idx]
            id_data += id_class_imgs
            # to avoid cuda assertion
            modified_class_idx = self.id_class_to_idx[class_idx]
            id_targets += [modified_class_idx for _ in range(len(id_class_imgs))]
        
        self.data, self.targets = id_data, id_targets
        
    def _images_by_class_index(self):
        # img_dict contains original dataset (including out-of-distribution)
        img_dict = {}
